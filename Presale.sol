...
contract PrivateSalePool is Ownable {
    ...
    uint256 constant _ONE_BNB_TO_TOKENS = 2500000000000 * 10**9;
    ...
    modifier notParticipate {
        require(
            _isParticipated[_msgSender()] == false,
            "User already participated"
        );
        _;
    }

    function purchase() public payable isWhitelisted notParticipate {
        require(msg.value == 1 ether, "Only allow to buy with 1 BNB");
        require(_totalParticipants < 100, "No tokens available to buy");

        uint256 amountOfTokensBuying = _ONE_BNB_TO_TOKENS;
        _totalTokensSold = _totalTokensSold.add(amountOfTokensBuying);
        _totalBNBRaised = _totalBNBRaised.add(1 ether);
    ...
    function totalBNBRaised() public view returns (uint256) {
        return _totalBNBRaised;
    }
